...
Question: Find out close_price against time using ggplot. 
```{r}
time <- a[["t"]]
close <- a[["c"]]
```
...
ggplot(aes(y = close_1, x = time_1)) +
  ...
  ylab("Close Price")
```
Interepretation: When time is around 1615300000, the close price is the lowest. Overall the close price is getting higher when the time goes. 
